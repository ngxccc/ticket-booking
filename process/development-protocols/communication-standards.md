...
- Wordy → tight: _"In order to utilize this you'll first need to make sure the cache is initialized."_ → **"Initialize the cache first."**
...
- Unexplained jargon → glossed: _"Fan-out to parallel subagents after PVL CONDITIONAL — check blast-radius overlap before EVL."_ → **"Spawn parallel agents after the plan is approved (PVL — plan-validate loop, passed) — check that they don't edit the same files before running the confirmation test run (EVL — execute-validate loop)."**
